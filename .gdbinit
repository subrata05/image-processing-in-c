...
#
#   BP 2 — line 292 (src/main.c)
#          The printf("Result: %dx%d\n", ...) right after any operation
#          (rotation or filter) completes.
#          At this point both srcImg AND dstImg are fully populated.
#          Use: inspect the processed image memory layout and compare.
#
# =============================================================================

break src/main.c:245
break src/main.c:292
...
# Use after BP 1 (line 245) or BP 2 (line 292)
# -----------------------------------------------------------------------------
define show_src
    printf "\n"
...
Available after breakpoint 1 (line 245) — right after bmpLoad().
end

# -----------------------------------------------------------------------------
# show_dst — memory layout of the PROCESSED/DESTINATION image (dstImg)
# Use after BP 2 (line 292) only — dstImg does not exist before processing
# -----------------------------------------------------------------------------
define show_dst
    printf "\n"
...
document show_dst
Show the full memory layout of the destination/processed image (dstImg).
Available after breakpoint 2 (line 292) — right after operation completes.
end

# -----------------------------------------------------------------------------
# compare — side by side comparison of src vs dst dimensions
# Use after BP 2 (line 292) only
# -----------------------------------------------------------------------------
define compare
    printf "\n"
...
# -----------------------------------------------------------------------------
# filter_info — list all filter types and aliases
# -----------------------------------------------------------------------------
define filter_info
    printf "\nAvailable Filter Types:\n"
    printf "  0  : FILTER_GRAYSCALE  (gray, grey, grayscale, greyscale, gs)\n"
    printf "  1  : FILTER_NEGATIVE   (neg, negative, invert, inv)\n"
    printf "-----------------------------------------------------------\n\n"
end
document filter_info
List all available filter types and their string aliases.
end
...
# These will show as "optimized out" or 0 until the program reaches line 240
...
printf "  BP 1 -> src/main.c:245  (right after bmpLoad    — srcImg is ready)\n"
printf "  BP 2 -> src/main.c:292  (right after operation  — dstImg is ready)\n"
printf "\n"
printf "Commands:\n"
printf "  show_src       Show source image full memory layout\n"
printf "  show_dst       Show destination/processed image memory layout\n"
printf "  compare        Side-by-side src vs dst comparison\n"
printf "  rotation_info  List all rotation types and aliases\n"
printf "  filter_info    List all filter types and aliases\n"
printf "\n"
printf "Workflow:\n"
printf "  run            Start the program\n"
printf "  (stops at BP1) show_src       <- source image layout\n"
printf "  continue\n"
printf "  (stops at BP2) show_dst       <- processed image layout\n"
